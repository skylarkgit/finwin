...
from finwin.server.routes.macro import router as macro_router, shutdown as macro_shutdown
...
async def get_context(
    symbol: Optional[str] = Query(
        None, description="Stock symbol (e.g., TCS.NS, AAPL)"
    ),
    query: Optional[str] = Query(
        None, description="Search query (e.g., company name)"
    ),
    news_count: int = Query(
        10, description="Number of news articles to fetch", ge=1, le=50
    ),
) -> StockContext:
    ...
    try:
        async with ContextCollector(
            ...
            return await collector.gather(symbol=symbol, query=query)
...
async def post_context(request: ContextRequest) -> StockContext:
    """Gather financial context (POST version with extra URLs)."""
